...
public abstract class Entity {
            ...
            if (Minecraft.getMinecraft().thePlayer != null) {
                try {
                    if ((ModManager.scaffold.isEnabled()
                            && ModManager.scaffold.safewalk.getValue()
                            && !Minecraft.getMinecraft().gameSettings.keyBindSneak.isKeyDown()
                            && Minecraft.getMinecraft().thePlayer.fallDistance < 2) || (ModManager.safeWalk.isEnabled()
                            && !Minecraft.getMinecraft().gameSettings.keyBindSneak.isKeyDown()
	...
	public void moveFlying(float strafe, float forward, float friction) {
		if (this == Minecraft.getMinecraft().thePlayer) {
            final EventStrafe strafeEvent = new EventStrafe(forward, strafe, friction);
            UwUClient.INSTANCE.modManager.onEvent(strafeEvent);
