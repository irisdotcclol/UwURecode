...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void onUpdateWalkingPlayer() {
		...
		UwUClient.INSTANCE.modManager.onEvent(preMotionEvent);
		...
		if (this.isCurrentViewEntity()) {
            ...
            boolean flag2 = d0 * d0 + d1 * d1 + d2 * d2 > (ModManager.no003.isEnabled() ? 0.0 : 9.0E-4D) || this.positionUpdateTicks >= 20;
			boolean flag3 = d3 != 0.0D || d4 != 0.0D;
			if (this.ridingEntity == null) {
				if (flag2 && flag3) {
					this.sendQueue.addToSendQueue(
							new C03PacketPlayer.C06PacketPlayerPosLook(preMotionEvent.getX(), preMotionEvent.getY(),
									preMotionEvent.getZ(), preMotionEvent.getYaw(), preMotionEvent.getPitch(), preMotionEvent.isGround()));
				} else if (flag2) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(preMotionEvent.getX(),
							preMotionEvent.getY(), preMotionEvent.getZ(), preMotionEvent.isGround()));
				} else if (flag3) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(preMotionEvent.getYaw(),
							preMotionEvent.getPitch(), preMotionEvent.isGround()));
				} else {
				...
				this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D,
						this.motionZ, preMotionEvent.getYaw(), preMotionEvent.getPitch(), preMotionEvent.isGround()));
				flag2 = false;
			}
		...
		UwUClient.INSTANCE.modManager.onEvent(postMotionEvent);
	}
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        if (!(this.noClip || (ModManager.freecam.isEnabled() && ModManager.freecam.noClip.getValue() && ModManager.freecam.fly.getValue()))) {
	...
	public void onLivingUpdate() {
		...
		boolean noslow = ModManager.noSlow.isEnabled();
		...
		if (this.onGround && !flag1 && !flag2 && (this.omniSprint || this.movementInput.moveForward >= f) && !this.isSprinting() && flag3
		...
		if (!this.isSprinting() && (this.omniSprint || this.movementInput.moveForward >= f)  && flag3 && (!this.isUsingItem() || noslow)
