...
import dev.uwuclient.event.impl.EventTick;
...
public class Minecraft implements IThreadListener {
	...
	public void runTick() throws IOException {
		...
		if (!this.isGamePaused) {
			...
			UwUClient.INSTANCE.modManager.onEvent(eventTick);
		}
		...
		if (this.currentScreen == null || this.currentScreen.allowUserInput) {
				...
				if (Keyboard.getEventKeyState()) {
					...
					if (this.currentScreen != null) {
						...
						for(Mod m : UwUClient.INSTANCE.modManager.modules){
							if(m.key == k){
								m.toggle();
								UwUClient.INSTANCE.notificationManager.registerNotification(
									(m.enabled ? "Enabled" : "Disabled") + " " + m.name, 2000, NotificationType.NOTIFICATION);
							}
						}

						if(k == KeyboardConstants.KEY_RSHIFT){
							ModManager.clickGuiMod.onEnable();
						}
			...
			CanPlaceBlockEvent e = new CanPlaceBlockEvent();
			e.call();
