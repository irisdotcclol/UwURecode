...
import static net.lax1dude.eaglercraft.v1_8.opengl.RealOpenGLEnums.GL_ALWAYS;
...
//			}
	...
	public void renderItemInFirstPerson(float partialTicks) {
		...
		if (this.itemToRender != null) {
			final boolean auraBlock = !ModManager.aura.blockMode.is("None") && Aura.target != null;

			if (this.itemToRender.getItem() == Items.filled_map) {
				this.renderItemMap(entityplayersp, f2, f, f1);
			} else if (entityplayersp.getItemInUseCount() > 0 || auraBlock) {
				...
				case DRINK:
					...
					case BLOCK: {
						if (!ModManager.animations.isEnabled()) {
							...
							final String mode = ModManager.animations.mode.getValue();
							...
							switch (mode) {
								...
								case "Skidding": {
									...
									final float ff = MathHelper.sin(f1 * f1 * (float) Math.PI);
									final float ff1 = MathHelper.sin(MathHelper.sqrt_float(f1) * (float) Math.PI);

									GlStateManager.rotate(ff * -30.0F, 0.0F, 1.0F, 0.0F);
									GlStateManager.rotate(ff1 * -20.0F, 0.0F, 0.0F, 1.0F);
									GlStateManager.rotate(ff1 * -85.0F, 1.0F, 0.0F, 0.0F);
								...
								case "Smooth": {
									final float convertedf1 = (float) Math.sin(Math.sqrt(f1) * Math.PI);

									this.transformFirstPersonItem(f, 0.0F);
									final float angle = -convertedf1 * 2;
									final float y = -convertedf1 * 2;
									GlStateManager.translate(0, y / 10 + 0.1f, 0);
									GlStateManager.rotate(y * 10, 0, 1, 0);
									GlStateManager.rotate(250, 0.2f, 1, -0.6f);
									GlStateManager.rotate(-10, 1, 0.5f, 1);
									GlStateManager.rotate(-angle * 20, 1, 0.5f, 1);
									break;
								}
								...
								case "Down":
									GlStateManager.translate(0.56F - f1 / 20, -0.40F + f1 / 40, -0.71999997F);
									GlStateManager.translate(0.0F, f1 * -0.6F, 0.0F - f1 / 25);
									GlStateManager.rotate(33.0F, 0.0F, 1.0F, 0.0F);

									final float ff = MathHelper.sin(f * f * (float) Math.PI);
									final float ff1 = MathHelper.sin(MathHelper.sqrt_float(f) * (float) Math.PI);

									GlStateManager.rotate(ff * 0.0F, 0.0F, 1.0F, 0.0F);
									GlStateManager.rotate(ff1 * -f1, 1.0F, 0.2F, 1.0F);
									GlStateManager.rotate(-10, 1, 0, 0);
									GlStateManager.rotate(ff1 * -f1, 1F, 0.4F, 0.9F);
									GlStateManager.scale(0.4F, 0.4F, 0.4F);
