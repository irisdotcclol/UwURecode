...
public class PlatformInput {
	...
	public static void pressAnyKeyScreen() {
		if(!getSiteInteraction() && !hasBeenActive()) {
			EarlyLoadScreen.paintEnable();
			
			while(!getSiteInteraction()) {
				EagUtils.sleep(100l);
			}
		}
	}

	@JSBody(params = { }, script = "return getSiteInteraction();" )
	private static native boolean getSiteInteraction();
